...
import { Calendar, Users, Map, Grid, CheckCircle, X, ShieldCheck, QrCode, Tent, ArrowRight, Sparkles, Send, PhoneCall, Search, Clock } from 'lucide-react';
...
export default function OnlineBookingPage() {
  ...
  const handleStayTypeChange = (type) => {
    ...
    if (type === 'overnight') {
      ...
      if (checkOutDate <= checkInDate) {
        setCheckOutDate(new Date(new Date(checkInDate).getTime() + 86400000).toISOString().split('T')[0]);
      }
    } else {
      setCheckInTime('14:00');
      setCheckOutTime('20:00');
      setCheckOutDate(checkInDate); // Default same day for day-use
    }
  };
  ...
  const handleCheckInChange = (val) => {
    ...
    if (stayType === 'dayuse') {
      setCheckOutDate(val);
    } else if (checkOutDate && val > checkOutDate) {
  ...
  const handleCheckOutChange = (val) => {
    if (stayType === 'overnight' && val < checkInDate) {
  ...
  const handleSearchAvailability = () => {
    fetchData(true);
    const formattedIn = new Date(`${checkInDate}T${checkInTime}`).toLocaleString('vi-VN', { dateStyle: 'short', timeStyle: 'short' });
    const formattedOut = new Date(`${checkOutDate}T${checkOutTime}`).toLocaleString('vi-VN', { dateStyle: 'short', timeStyle: 'short' });
    toast.success(`Đã cập nhật lều trống cho khung giờ ${formattedIn} ➔ ${formattedOut}`);
  };
  ...
  const effectiveTents = tents.map(tent => {
    const activeBooking = tent.bookings?.find(b => {
      if (b.status === 'CheckedOut' || b.status === 'Cancelled' || b.status === 'Rejected') return false;
    ...
    const status = activeBooking ? (activeBooking.status || tent.status) : 'Available';
  ...
  const handleSelectTent = (tent) => {
    if (tent.status !== 'Available') {
      toast.error(`Lều ${tent.name} hiện đã được đặt trong khoảng thời gian này.`);
      return;
    }
    ...
    if (exists) {
      ...
      toast.success(`Đã chọn Lều ${tent.name}! Click thêm lều khác hoặc bấm "Tiếp tục" bên dưới.`, { id: 'select-tent-toast' });
    }
  };
  ...
  const totalPricePerNight = selectedTents.reduce((sum, t) => sum + (t.price || 0), 0);
  const grandTotal = totalPricePerNight * nights;

  const handleSubmitRequest = async (e) => {
    e.preventDefault();
    if (selectedTents.length === 0) return toast.error("Vui lòng chọn ít nhất 1 lều!");
    if (!customerName || !phoneNumber) return toast.error("Vui lòng điền Họ tên và Số điện thoại!");
    ...
    try {
      const res = await axios.post(getApiUrl('/api/Bookings/online-booking-request'), {
        customerName,
        phoneNumber,
      ...
      <div className="bg-[#1B4D3E] text-white pt-10 pb-16 px-4 sm:px-8 relative overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight">Sơ Đồ Bãi Cắm Trại Bùi Hui</h1>
            <p className="text-emerald-100 text-sm mt-1 max-w-xl">
              Chọn khoảng thời gian lưu trú và nhấp trực tiếp vào điểm lều trên bản đồ 2D để xem giá và đặt cọc nhanh chóng.
            </p>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md border border-slate-200 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
            ...
            <div className="text-xs font-bold text-slate-500">
              {stayType === 'overnight' ? (
                <span>⏰ Nhận lều <strong>14:00</strong> & Trả lều trước <strong>12:00 (Trưa hôm sau)</strong></span>
              ) : (
                <span>⏱️ Thuê linh hoạt trong ngày ➔ Chọn khoảng giờ nhận & trả bên dưới</span>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                <Calendar size={14} className="text-emerald-600" /> {stayType === 'overnight' ? 'Ngày Check-in' : 'Ngày Ở'}
              </label>
              <input 
                type="date" 
                value={checkInDate}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                <Calendar size={14} className="text-emerald-600" /> {stayType === 'overnight' ? 'Ngày Check-out' : 'Ngày Trả Lều'}
              </label>
              <input 
                type="date" 
                value={checkOutDate}
        ...
        <div className="space-y-10">
          {effectiveZones.map((zone) => {
            const zoneTents = (zone.tents || []).map((t) => {
              const liveTent = effectiveTents.find((et) => et.id === t.id) || t;
              return liveTent;
            });
              ...
              <div key={zone.id} className="space-y-4">
                ...
                <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
                  ...
                  <span className="text-xs font-black text-emerald-800 bg-emerald-50 px-3.5 py-1.5 rounded-full border border-emerald-200">
                    {zoneTents.filter(t => t.status === 'Available').length} / {zoneTents.length} lều trống
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {zoneTents.map((tent) => {
                    ...
                    const isSelected = selectedTents.some(t => t.id === tent.id);
                    ...
                    return (
                      <div 
                        ...
                        className={`bg-white rounded-3xl p-6 shadow-sm border transition-all duration-300 cursor-pointer flex flex-col justify-between hover:shadow-xl ${
                          isSelected ? 'border-amber-500 ring-2 ring-amber-400 bg-amber-50/20' :
                          isAvailable ? 'border-slate-200 hover:border-emerald-500' : 
                          isPending ? 'border-amber-200 bg-amber-50/30' : 'border-slate-100 opacity-60'
                        }`}
                      >
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase ${
                              ...
                              {isSelected ? 'Đang chọn' : isAvailable ? 'Trống' : isPending ? 'Chờ Xác Nhận' : 'Đã Đặt'}
                            </span>
                          </div>
                          <h3 className="text-xl font-extrabold text-slate-800 mb-1">Lều {tent.name}</h3>
                          <p className="text-xs text-slate-500 mb-4">Săn mây, view núi rừng thoáng mát</p>
                        </div>
                        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                          <div>
                            <span className="text-[10px] font-bold text-slate-400 block uppercase">Giá thuê/đêm</span>
                            <span className="text-lg font-black text-emerald-600">
                              {tent.price ? tent.price.toLocaleString('vi-VN') + 'đ' : '0đ'}
                            </span>
                          </div>
                          <button className={`p-2.5 rounded-xl shadow-md transition-all ${isSelected ? 'bg-amber-500 text-white' : 'bg-[#1B4D3E] text-white hover:bg-emerald-800'}`}>
                            <ArrowRight size={18} />
                          </button>
                        </div>
                      </div>
      ...
      {selectedTents.length > 0 && !isDrawerOpen && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-[#1B4D3E]/95 backdrop-blur-xl text-white px-6 py-4 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/20 flex items-center gap-6 animate-in slide-in-from-bottom duration-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-amber-300 uppercase tracking-wider">Đã chọn {selectedTents.length} lều</p>
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-white/20">
            <div className="text-right hidden sm:block">
              <span className="text-[10px] text-emerald-200 block uppercase font-medium">Dự kiến ({nights} đêm)</span>
              <span className="text-sm font-black text-amber-300">{grandTotal.toLocaleString('vi-VN')}đ</span>
            </div>
            <button 
          ...
          <div className="bg-white w-full max-w-lg h-[calc(100vh-64px)] rounded-tl-3xl shadow-2xl p-6 sm:p-8 flex flex-col justify-between overflow-y-auto border-l border-t border-slate-200 animate-in slide-in-from-right duration-300">
            <div>
              ...
              <div className="relative aspect-video rounded-2xl overflow-hidden mb-6 shadow-md border border-slate-100">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent flex items-end p-4 text-white">
                  <div>
                    ...
                    <p className="text-base font-black mt-1">Bùi Hui Camping Peak</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmitRequest} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Họ & Tên Người Đặt (*)</label>
                  <input 
                    required 
                    type="text" 
                    value={customerName}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Số Điện Thoại Zalo/Liên Hệ (*)</label>
                  <input 
                    required 
                    type="tel" 
                    value={phoneNumber}
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 space-y-3">
                  <div className="flex justify-between text-xs font-medium text-slate-600">
                    ...
                    <span className="font-bold text-slate-800">{checkInDate} ➔ {checkOutDate} ({nights} đêm)</span>
                  </div>

                  <div className="space-y-2 pt-2 border-t border-slate-200">
                    <span className="text-[11px] font-bold text-slate-500 uppercase block">Chi Tiết Khu Vực & Lều Đã Chọn ({selectedTents.length}):</span>
                    {selectedTents.map(t => (
                      <div key={t.id} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 text-xs shadow-sm">
                        <div>
                          <span className="text-emerald-700 font-extrabold block text-[11px]">{t.zone?.name || 'Khu Cắm Trại'}</span>
                          <span className="text-slate-800 font-black text-sm">Lều {t.name}</span>
                        </div>
                        <span className="font-extrabold text-[#1B4D3E] bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200/60">
                          {t.price ? t.price.toLocaleString('vi-VN') + 'đ' : '0đ'} / đêm
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-200 text-sm font-extrabold text-slate-800">
                    ...
                    <span className="text-[#1B4D3E] text-base">{grandTotal.toLocaleString('vi-VN')}đ</span>
                  </div>
                </div>
                ...
                <div className="bg-emerald-50 border border-emerald-200 p-3.5 rounded-2xl text-xs text-emerald-900 font-medium leading-relaxed">
                  💡 <strong>Quy trình giữ lều:</strong> Sau khi gửi yêu cầu, Lễ tân Bùi Hui sẽ liên hệ SĐT/Zalo <strong>{phoneNumber || 'của bạn'}</strong> để tư vấn chi tiết và hỗ trợ nhận cọc chốt giữ lều.
                </div>
                ...
                >
                  <Send size={20} />
                  {isSubmitting ? 'Đang gửi yêu cầu...' : `Gửi Yêu Cầu Đặt ${selectedTents.length} Lều`}
                </button>
              </form>
            </div>
          </div>
      ...
      {bookingSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md pt-16 animate-in fade-in">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center space-y-6 shadow-2xl border border-slate-100 animate-in zoom-in-95">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-800">Đã Gửi Yêu Cầu Đặt Lều!</h2>
              <p className="text-xs text-slate-500 mt-1">Yêu cầu giữ lều của bạn đã được gửi đến Lễ Tân Bùi Hui</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-left text-xs space-y-2 font-medium">
              <p><span className="text-slate-400">Các lều chọn:</span> <strong className="text-emerald-700 font-bold">Lều {bookingSuccess.tentsList}</strong></p>
              <p><span className="text-slate-400">Thời gian:</span> <strong className="text-slate-800">{bookingSuccess.checkInDate} ➔ {bookingSuccess.checkOutDate}</strong></p>
              <p><span className="text-slate-400">Khách hàng:</span> <strong className="text-slate-800">{bookingSuccess.customerName}</strong></p>
              <p><span className="text-slate-400">Số điện thoại:</span> <strong className="text-slate-800">{bookingSuccess.phoneNumber}</strong></p>
              <p className="pt-2 text-amber-700 font-bold border-t border-slate-200">
                📞 Lễ tân sẽ sớm liên hệ với bạn để hỗ trợ gửi mã QR nhận cọc & chốt đơn!
              </p>
            </div>
            <button 
              onClick={() => { setBookingSuccess(null); setSelectedTents([]); }}
